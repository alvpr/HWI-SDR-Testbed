...
class qam16_hardware(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "qam16_hardware", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("qam16_hardware")
        ...
        self.settings = Qt.QSettings("GNU Radio", "qam16_hardware")
        ...
        # Variables
        ##################################################
        self.const_base = const_base = digital.qam_constellation(16,True,digital.mod_codes.GRAY_CODE,False)
        ...
        self.mod_avg = mod_avg = np.sum(np.abs(pconst))/len(pconst)
        self.gain_tx = gain_tx = 0
        self.gain_rx = gain_rx = 0
        self.freq = freq = 900e6
        self.d = d = 1/np.sqrt(2)
        self.const_iq = const_iq = digital.constellation_calcdist(pconst_iq, [i for i in range(16)],
        1, 1, digital.constellation.NO_NORMALIZATION).base()
        self.const = const = digital.constellation_calcdist(pconst, [i for i in range(16)],
        1, 1, digital.constellation.NO_NORMALIZATION).base()
        self.arity = arity = 4
        ...
        self.demonstrator_crc32_check_0 = demonstrator.crc32_check(10000, 200, 1)
        ...
        self.analog_agc_xx_0_0 = analog.agc_cc(1e-4, mod_avg_iq*0.98, 1.0)
    ...
    def set_pconst(self, pconst):
        self.pconst = pconst
        self.set_mod_avg(np.sum(np.abs(self.pconst))/len(self.pconst))
    ...
    def set_mod_avg_iq(self, mod_avg_iq):
        ...
        self.analog_agc_xx_0_0.set_reference(self.mod_avg_iq*0.98)

    def get_mod_avg(self):
        return self.mod_avg

    def set_mod_avg(self, mod_avg):
        self.mod_avg = mod_avg
...
def main(top_block_cls=qam16_hardware, options=None):
